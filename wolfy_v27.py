...
def validate_ip_or_hostname(value):
    """
    Validates if the given value is an IP address or a hostname.
    - IP address: Matches IPv4 format.
    - Hostname: Alphanumeric with optional hyphens and dots, limited to 253 characters.
    """
    ip_pattern = re.compile(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$")
    hostname_pattern = re.compile(r"^(?!-)[A-Za-z0-9-]{1,63}(?<!-)(\.[A-Za-z0-9-]{1,63})*$")
    
    return bool(ip_pattern.match(value) or hostname_pattern.match(value))
...
def main():
    ...
    with open(file_path, 'r') as file:
        ip_or_hostnames = [line.strip() for line in file if line.strip()]

    if len(ip_or_hostnames) != number_of_ips:
        print(f"Error: The number of entries in the file ({len(ip_or_hostnames)}) does not match the specified number of entries ({number_of_ips}).")
        return

    duplicates = check_for_duplicates(ip_or_hostnames)
    if duplicates:
        print("Duplicate IPs/Hostnames found:")
        for value, positions in duplicates.items():
            print(f"{value} appears at positions {positions}")
        user_input = get_input("\nDo you want to proceed with the batch division? (Y or yes to proceed, any other key to exit): ").lower()
        if user_input not in ('y', 'yes'):
            print("Exiting the tool.")
            return

    for value in ip_or_hostnames:
        if not validate_ip_or_hostname(value):
            print(f"Error: Invalid IP address or hostname detected: {value}")
            return
    ...
    print(f"\nIP addresses/hostnames saved to {host_ips_file}")

    batch_files = [host_ips_file]
    if len(ip_or_hostnames) > 6:
        num_batches = get_number_of_batches()
        batches = divide_ips_into_batches(ip_or_hostnames, num_batches)
